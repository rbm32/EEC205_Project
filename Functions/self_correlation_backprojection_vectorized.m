function reconstructed_image = self_correlation_backprojection_vectorized(bscan, x, time, c, h, epsilon_r, image_size, alpha, correlation_threshold)
    ...
    for m = 1:num_antennas
        for i = 1:image_size
            for j = 1:image_size
                x0 = X(i, j);
                z0 = Z(i, j);
                tof_lookup(i, j, m) = compute_tof(x(m), h, x0, z0, epsilon_r, c, false);
            end
        end
    end
