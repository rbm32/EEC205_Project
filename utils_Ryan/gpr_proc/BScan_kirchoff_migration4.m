function I = BScan_kirchoff_migration4(C, h, er, t, x, tx_pos, rx_pos, xQ, zQ, progress)
% Sam W. UCD MML
% Small edits made by Tyler. K. for bug improvements.
...
for ii = 1:NxQ
    ...
    indexing_matrix = xa_lower_ind + (0:(size_left+size_right));

    max_index = size(C,2);

    indexing_matrix(indexing_matrix > max_index) = max_index;


    C11 = C(:,indexing_matrix);
    ...
    if(progress)
        progressbar(ii/NxQ); % let 'em know!
    end
end
