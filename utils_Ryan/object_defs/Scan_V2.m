classdef Scan_V2 < handle
    ...
    methods
        ...
        % S.read_scope_data(obj, fn, fn_meta)
        %   | reads scope and metadata at fn
        ...
        function obj = read_scope_data(obj, fn, fn_meta)
            arguments
                obj;
                fn string;
                fn_meta = fn{1}(1:end-4) + "-meta_x_t_times.csv";

            end

            assert(isfile(fn), fn + " does not exist.");
            assert(isfile(fn_meta), fn_meta + " does not exist.");

            data = csvread(fn);
            meta = csvread(fn_meta);
            
            obj.D = data;
            obj.N_x = size(obj.D, 2);
            obj.t = meta(2,:)
            x_vals = meta(1,:);
            obj.x = x_vals(x_vals ~=0);
...
%             plot(cal_wfm,'r');
        ...
        function obj = setup_imaging_params(obj,pixel_width,zlims,xlims,h,er,tx_pos,rx_pos)
            ...
            obj.xC          = min(xlims):pixel_width/6:max(xlims);
        ...
        function obj = migrate(obj)
            obj.I = BScan_kirchoff_migration4(obj.D_BKGR, obj.h, obj.er, obj.t, obj.x, obj.tx_pos, obj.rx_pos, obj.xC, obj.zC, true);
        end
        ...
        function obj = migrate_MF(obj,progress)
            ...
            obj.I_MF   = BScan_kirchoff_migration4(obj.D_BKGR, obj.h, obj.er, obj.t, obj.x, obj.tx_pos, obj.rx_pos, obj.xC, obj.zC, true);
...
%             end
        ...
        function obj = plot_Ryan(obj, plot_type, tl, DR)
            arguments
                obj;
                plot_type string;
                tl string;
            ...
            switch plot_type
                ...
                case "Migrated MF"
                    ...
                    yL    = "z (cm)";
                    ydir  = 'reverse';
                    clims = [max(I_mat(:))*DR, max(I_mat(:))];
            end

            % imagesc(x_vec, y_vec, lowpass(I_mat, .2));
        ...
        function obj = crop(obj, t_crop, x_crop)
            arguments
                obj;
                t_crop = [-inf inf];
                x_crop = [-inf inf];
            ...
            t_idx = and(obj.t>t_crop(1), obj.t<t_crop(2));
            x_idx = and(obj.x>x_crop(1), obj.x<x_crop(2));
            % obj.x = obj.x(x_idx);
            obj.t = obj.t(t_idx);
            obj.D = obj.D(t_idx, x_idx);
